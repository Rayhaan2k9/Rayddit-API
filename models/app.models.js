...
exports.deleteCommentById = (comment_id) => {
    ...
    if(regex.test(comment_id) === false) {
        return Promise.reject({
            status: 400, message: 'Please enter a valid comment_id' 
        })
    }

    return db.query(`SELECT * FROM comments WHERE comment_id = ${comment_id}`)
    .then((comments) => {
        if (comments.rows.length === 0) {
            return Promise.reject({
                status: 404, message: `No comment found for comment_id: ${comment_id}`
            })
        }
    })
    .then(() => {

  ...
  describe("GET: Error handling", () => {
    test("400: Bad request", () => {
      return request(app)
        ...
        .then((res) => {
          expect(res.body.message).toBe(
            "Bad request, please enter a valid number"
          );
        });
    });
    ...
    test("404: article not found", () => {
      return request(app)
        ...
        .then((res) => {
          expect(res.body.message).toBe(
            "No article found for article_id: 99999"
          );
        });
    });
  });
  describe("POST: Happy path", () => {
    test.only("201: returns posted comment", () => {
      return request(app)
  ...
  describe("POST: Error handling", () => {
    ...
    test("404: article does not exist, when article does not exists under specified id", () => {
      return request(app)
        ...
        .then((res) => {
          expect(res.body.message).toBe(
            "No article found for article_id: 1234"
          );
        });
    });
  ...
  describe("DELETE: Error handling", () => {
    ...
    test("404: comment does not exist under specified id when number is entered but no comment with that id", () => {
      return request(app)
        ...
        .then((res) => {
          expect(res.body.message).toBe(
            "No comment found for comment_id: 1000"
          );
        });
    });
  });
